...
import { SensorMinMax } from "./Sensors"
...
export default function(eep: string, data: string){
	...
	if (choice === "a5" && func === "02" && !!sensors[type]) {
		return [{
			type: "temperature",
			unit: "°C",
			value: Help.extractByteValue(1, 255, 0, sensors[type].min, sensors[type].max, data)
		}]
	}
	return ret
}

const sensors: SensorMinMax = {
	"01": { min: -40, max: 0 }, //1
	"02": { min: -30, max: 10 }, //2
	"03": { min: -20, max: 20 }, //3
	"04": { min: -10, max: 30 }, //4
